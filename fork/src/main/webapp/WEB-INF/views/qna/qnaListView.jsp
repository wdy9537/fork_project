    ...
    <div id="container">
    	<c:choose>
    		<c:when test="${loginUser.memberId == 'admin' }">
		        <div id="notice_title1">Q&A 관리</div>	
    		</c:when>
    		<c:otherwise>
		        <div id="notice_title1">Q&A</div>	
    		</c:otherwise>
    	</c:choose>
        ...
        <table>
            ...
            <tbody>
            	...
            	<c:forEach var="q" items="${list }">
	                <tr> 
	                	<c:if test="${loginUser.memberId == 'admin' }">
	                		<td><input type="checkbox" name='checkbox' value="${q.boardNo}" data-notice-id="${q.boardNo}"></td>
	                	</c:if>
	                    <td onclick="movePage(${q.boardNo});">${q.boardNo }</td>
	                    <td onclick="movePage(${q.boardNo});">${q.boardTitle }</td>
	                    <td onclick="movePage(${q.boardNo});">${q.boardWriter}</td>
	                    <td onclick="movePage(${q.boardNo});">${q.boardCreateDate }</td>
	                    <td onclick="movePage(${q.boardNo});">${q.boardCount }</td>
	                </tr>
            	</c:forEach>
            </tbody>
        </table>
	        ...
	        <div class="notice_btn_container">
	        	<c:choose>
	        		<c:when test="${loginUser.memberId == 'admin' }">
	    				...
	    				<div><button type="submit" class="notice_btn_write" onclick="qnaEnroll();">글쓰기</button></div>
	        		</c:when>
	        		<c:when test="${empty loginUser}">
	        			<div><button type="submit" class="notice_btn_delete" onclick="deleteValue();" style="visibility: hidden;">선택삭제</button></div>
	    				<div><button type="submit" class="notice_btn_write" onclick="qnaEnroll();" style="visibility: hidden;">글쓰기</button></div>
	        		</c:when>
	        		<c:when test="${loginUser.memberId != 'admin' }">
	        			<div><button type="submit" class="notice_btn_delete" onclick="deleteValue();" style="visibility: hidden;">선택삭제</button></div>
	    				<div><button type="submit" class="notice_btn_write" onclick="qnaEnroll();">글쓰기</button></div>
...
<script>
   ...
   <!-- 게시글 상세페이지로 이동 -->
  	function movePage(no){
  		location.href="${contextPath}/qnaDetail/"+no;
  	}
  	
   <!-- 글쓰기 페이지로 이동 -->
  	function qnaEnroll() {
    	location.href = "${contextPath}/qnaEnroll";
	}
	                    ...
	                    success: function (jdata) {
	                        if (jdata === 1) {
	                            ...
	                            alertify.alert(" ", "삭제 완료", function () {
	                                window.location.href = "/fork/qnaList";
	                            });
	                        }
	                    }
	                });
